...
- [-] 4. Implementar entidades e interfaces de domínio
  - [x] 4.1 Criar `crypto_result.dart` com a entidade `CryptoResult(String value)`
    - _Requirements: 1.4, 2.4_

  - [-] 4.2 Criar `crypto_repository.dart` com a interface abstrata `CryptoRepository` declarando `encrypt(String text)` e `decrypt(String bytes)` retornando `Future<Either<Failure, CryptoResult>>`
    - _Requirements: 1.3, 1.4, 1.5, 2.3, 2.4, 2.5_

- [-] 5. Implementar use cases
  - [-] 5.1 Criar `encrypt_use_case.dart` que delega para `CryptoRepository.encrypt(text)`
    - _Requirements: 1.3, 1.4, 1.5_

  - [-] 5.2 Criar `decrypt_use_case.dart` que delega para `CryptoRepository.decrypt(bytes)`
...
- [~] 6. Implementar `CryptoRemoteDataSource`
  - [~] 6.1 Criar `crypto_remote_data_source.dart` com a interface abstrata e a implementação que usa `Dio` para `POST` nos endpoints de encrypt e decrypt
...
- [~] 7. Implementar `CryptoRepositoryImpl`
  - [~] 7.1 Criar `crypto_repository_impl.dart` implementando `CryptoRepository`
...
- [~] 8. Checkpoint — Verificar camadas de dados e domínio
  - Garantir que todos os testes das camadas Data e Domain passam. Perguntar ao usuário se houver dúvidas antes de prosseguir.

- [~] 9. Implementar `CryptoCubit` e `CryptoState`
  - [~] 9.1 Criar `crypto_state.dart` com a sealed class `CryptoState` e subclasses: `CryptoInitial`, `CryptoLoading`, `CryptoSuccess(String result)`, `CryptoFailure({String message, bool canRetry})`
    - _Requirements: 1.7, 2.7, 7.1, 7.2, 7.3_

  - [~] 9.2 Criar `crypto_cubit.dart` com `CryptoCubit` contendo os métodos `encrypt(String text)`, `decrypt(String bytes)`, `retry()` e `clear()`
...
- [~] 10. Implementar widgets de apresentação
  - [~] 10.1 Criar `error_banner.dart` que exibe a mensagem de erro e, quando `canRetry = true`, o botão "Tentar Novamente" que chama `onRetry`
    - _Requirements: 7.1, 7.2, 7.3_

  - [~] 10.2 Criar `result_card.dart` que exibe o resultado e o botão "Copiar"; ao pressionar "Copiar", usa `ClipboardHelper` e exibe um `SnackBar` de confirmação
    - _Requirements: 3.4, 3.5_

  - [~] 10.3 Criar `crypto_tab.dart` como widget reutilizável com campo de entrada, botão de ação (Criptografar/Descriptografar), botão "Limpar" e `BlocBuilder<CryptoCubit, CryptoState>` para reagir aos estados
  ...
  - [~] 10.4 Criar `crypto_page.dart` com `TabBar` de duas abas ("Criptografar" e "Descriptografar"), cada aba com seu próprio `BlocProvider<CryptoCubit>` para manter estados independentes
    - _Requirements: 3.1_

  - [~] 10.5 Criar `app.dart` com `MaterialApp`, tema e rota inicial para `CryptoPage`
    - _Requirements: 3.1_

  - [~] 10.6 Atualizar `main.dart` para inicializar o app via `app.dart`
    - _Requirements: 3.1_
...
- [~] 12. Configurar ícones adaptativos Android
  ...
  - [~] 12.1 Copiar `logo.png` da raiz do projeto para `assets/icons/icon_foreground.png` (camada foreground do ícone adaptativo); criar também `assets/icons/icon_background.png` com uma cor sólida ou versão de fundo derivada da logo; declarar o diretório `assets/icons/` no `pubspec.yaml`
  ...
  - [~] 12.2 Configurar a seção `flutter_launcher_icons` no `pubspec.yaml` apontando para os arquivos gerados a partir da `logo.png`: `image_path`, `adaptive_icon_background`, `adaptive_icon_foreground` e `min_sdk_android: 21`
    - _Requirements: 4.2, 4.4_

  - [~] 12.3 Executar `dart run flutter_launcher_icons` para gerar automaticamente todos os tamanhos e densidades de ícone exigidos (`mdpi`, `hdpi`, `xhdpi`, `xxhdpi`, `xxxhdpi`) em `android/app/src/main/res/`, usando a `logo.png` como origem
    - _Requirements: 4.2, 4.4_

- [~] 13. Configurar build Android release (assinatura, R8 e AAB)
  - [~] 13.1 Gerar o Keystore `upload-keystore.jks` com RSA 2048 bits e validade mínima de 25 anos usando `keytool`
    - _Requirements: 5.1_

  - [~] 13.2 Criar `android/key.properties` com `storePassword`, `keyPassword`, `keyAlias` e `storeFile`; adicionar `key.properties` ao `.gitignore`
    - _Requirements: 5.2, 5.3_

  - [~] 13.3 Atualizar `android/app/build.gradle` para:
  ...
  - [~] 13.4 Criar/atualizar `android/app/proguard-rules.pro` com regras para preservar classes do app, Dio/OkHttp e Flutter runtime
    - _Requirements: 6.2_

  - [~] 13.5 Verificar que `android/app/build.gradle` exibe mensagem de erro descritiva quando `key.properties` está ausente (bloco condicional no Gradle)
    - _Requirements: 6.5_
